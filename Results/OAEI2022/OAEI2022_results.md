...
We performed the evaluation using the MSE benchmark v1.1 and the MELT platform on a Windows 10 system with 16 GB RAM. For the time being, no background knowledge was used for evaluation. Every participating system was executed in its standard setting and we compare precision, recall and F-measure as well as the computation time. 
